	...
	public String customerDashboard(Model model,HttpSession session) {
		...
		if(optional.isPresent()) {
					...
					case "CUSTOMER":
						...
						//Check if customer's account is created or not
						CustomerVO customerVO = customerAccountService.findByUsername(username);
						if(customerVO.getStatus().equals("PENDING")) {
							model.addAttribute("message", "Dear Customer, Your Account is not yet created by the admin");
							viewName = "customer/accountPending";
						}
