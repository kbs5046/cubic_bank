...
public class SecurityQuestionServiceImpl implements SecurityQuestionService {
	...
	@Override
	public CustomerSecurityQueAnsVO findAnsByLoginid(String username) {
		...
		queAnsVO.setSecurityQuestionAnswer1(customerQuestionAnswers.get(0).getAnswer());
		queAnsVO.setSecurityQuestionAnswer2(customerQuestionAnswers.get(1).getAnswer());
		return queAnsVO;
	}
	
	@Override
	public CustomerSecurityQueAnsVO getQuestionByLoginid(String username) {
		List<CustomerQuestionAnswer> customerQuestionAnswers=customerQuestionsAnsRepository.findQuestionAnswer(username);
		CustomerSecurityQueAnsVO queAnsVO = null;
		if(customerQuestionAnswers.size()>0) {
			queAnsVO = new CustomerSecurityQueAnsVO();
			queAnsVO.setSecurityQuestion1(customerQuestionAnswers.get(0).getQuestion());
			queAnsVO.setSecurityQuestion2(customerQuestionAnswers.get(1).getQuestion());
			queAnsVO.setLoginid(customerQuestionAnswers.get(0).getLogin().getLoginid());
		}
		return queAnsVO;
	}
	...
	public List<SecurityQuestionsVO>  findAll(){
		List<SecurityQuestions>  securityQuestions=questionsRepository.findActiveQuestions("yes");
...
//		List<SecurityQuestionsVO> questionsVOs=new ArrayList<>();
//		for(SecurityQuestions questions:securityQuestions) {
//			if(questions.getStatus().equals("yes")) {
//				SecurityQuestionsVO questionsVO=new SecurityQuestionsVO();
//				BeanUtils.copyProperties(questions, questionsVO);
//				questionsVOs.add(questionsVO);
//			}
//		}
	...
	@Override
	public SecurityQuestionsVO  findByid(int id){
		SecurityQuestions  securityQuestion=questionsRepository.findById(id).get();
		SecurityQuestionsVO questionsVO=new SecurityQuestionsVO();
		BeanUtils.copyProperties(securityQuestion, questionsVO);
		return questionsVO;
	}
